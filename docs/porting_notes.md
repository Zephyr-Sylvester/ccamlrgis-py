...
## 8. `clip_to_coast`'s default `coast` argument (RESOLVED)
...
**Python behaviour:** `clip_to_coast(input, coast=None)` now defaults to
`ccamlrgis.datasets.load_example("Coast")`, matching R's default, once §4's
data-publishing pipeline (`tools/build_datasets.py` + a GitHub Release)
existed to make that cache-backed download possible. `coast=` can still be
passed explicitly (e.g. `load_coastline()`, which the R docs themselves
recommend for accuracy) to override it.

**History (why this was a temporary deviation before the data pipeline
existed):** under the download-on-demand model (§1.3), R's bundled `Coast`
had to become a cache-backed download, which needed §4's pipeline built
first. Rather than block `clip_to_coast` entirely until then, or silently
default to something that wasn't actually R's `Coast` dataset, the argument
was required in the meantime. Resolved once `datasets.py`/
`tools/build_datasets.py` landed.

**Impact on users:** `clip_to_coast(polys)` now works with no `coast=`
argument, matching R, and triggers a cache-backed download of `Coast` on
first use (or `force_refresh=` on `load_example` to bypass the cache).
