...
| `add_Cscale(pos='1/1', title='Depth (m)', width=18, height=70, cuts=Depth_cuts, cols=Depth_cols, ...)` | `ccamlrgis.plot.add_colour_scale(ax, cuts=None, cols=None, title='Depth (m)', loc='right', size='5%', pad=0.15, fontsize=10, ...)` | `pos='1/1'` base-graphics grid string replaced by an edge `loc=` ("right"/"left"/"top"/"bottom") placed via `make_axes_locatable` (shrinks `ax` to make room, matching R's non-overlapping result) — see `porting_notes.md` deviation 4 |
